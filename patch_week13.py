...
# Remove any existing week 13 code if we partially added it
if 'if (!existingWeeks.some(w => w.number === 13)) {' in content:
    content = content[:content.find('if (!existingWeeks.some(w => w.number === 13)) {')] + content[content.rfind('}', 0, content.find('seed().catch')):]
...
if brace_before_catch != -1:
    content = content[:brace_before_catch] + week13 + "\n" + content[brace_before_catch:]
    with open('db/seed.ts', 'w') as f:
        f.write(content)
    print("Successfully patched db/seed.ts for Week 13")
else:
